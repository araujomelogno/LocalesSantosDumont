package uy.com.innobit.rem.presentation.view.reports;
...
import uy.com.innobit.rem.persistence.datamodel.dashboardemo.Transaction;
import uy.com.innobit.rem.presentation.component.InlineTextEditor;
import uy.com.innobit.rem.presentation.component.TopSixTheatersChart;
import uy.com.innobit.rem.presentation.component.TopTenMoviesTable;
import uy.com.innobit.rem.presentation.component.TransactionsListing;
...
public final class ReportEditor extends VerticalLayout {
	...
	/*
	 * private Component buildPalette() { HorizontalLayout paletteLayout = new
	 * HorizontalLayout(); paletteLayout.setSpacing(true);
	 * paletteLayout.setWidthUndefined(); paletteLayout.addStyleName("palette");
	 * 
	 ...
	 * 
	 * @Override public void layoutClick(final LayoutClickEvent event) { if
	 * (event.getChildComponent() != null) { PaletteItemType data =
	 * (PaletteItemType) ((DragAndDropWrapper) event
	 * .getChildComponent()).getData(); addWidget(data, null); } } });
	 * 
	 * return paletteLayout; }
	 * 
	 * private Component buildPaletteItem(final PaletteItemType type) { Label
	 * caption = new Label(type.getIcon().getHtml() + type.getTitle(),
	 * ContentMode.HTML); caption.setSizeUndefined();
	 * 
	 * DragAndDropWrapper ddWrap = new DragAndDropWrapper(caption);
	 * ddWrap.setSizeUndefined();
	 * ddWrap.setDragStartMode(DragStartMode.WRAPPER); ddWrap.setData(type);
	 * return ddWrap; }
	 */

	public void addWidget(final PaletteItemType paletteItemType, final Object prefillData) {
	...
	public final class SortableLayout extends CustomComponent {
			...
			placeholder.setDropHandler(new DropHandler() {
				...
				public void drop(final DragAndDropEvent event) {
					...
					if (sourceComponent != layout.getParent()) {
						Object type = ((AbstractComponent) sourceComponent).getData();
		...
		public void addComponent(final PaletteItemType paletteItemType, final Object prefillData) {
			if (placeholder.getParent() != null) {
				layout.removeComponent(placeholder);
			}
			layout.addComponent(new WrappedComponent(createComponentFromPaletteItem(paletteItemType, prefillData)), 1);
		}

		private Component createComponentFromPaletteItem(final PaletteItemType type, final Object prefillData) {
			Component result = null;
			if (type == PaletteItemType.TEXT) {
				result = new InlineTextEditor(prefillData != null ? String.valueOf(prefillData) : null);
				...
				result = new TransactionsListing((Collection<Transaction>) prefillData);
			}
		...
		private class ReorderLayoutDropHandler implements DropHandler {
			...
			public void drop(final DragAndDropEvent dropEvent) {
				...
				if (sourceComponent.getParent() != layout) {
					Object paletteItemType = ((AbstractComponent) sourceComponent).getData();

					AbstractComponent c = new WrappedComponent(
							createComponentFromPaletteItem((PaletteItemType) paletteItemType, null));
					...
					if (dropTargetData.getData("verticalLocation").equals(VerticalDropLocation.TOP.toString())) {
						index--;
						if (index <= 0) {
							index = 1;
						}
	...
	public enum PaletteItemType {
		TEXT("Text Block", FontAwesome.FONT), TABLE("Top 10 Movies", FontAwesome.TABLE), CHART("Top 6 Revenue",
				FontAwesome.BAR_CHART_O), TRANSACTIONS("Latest transactions", null);
