...
import uy.com.innobit.rem.presentation.event.DashboardEvent.ReportsCountUpdatedEvent;
import uy.com.innobit.rem.presentation.event.DashboardEventBus;
import uy.com.innobit.rem.presentation.view.reports.ReportEditor.ReportEditorListener;

@SuppressWarnings("serial")
public final class ReportsView extends TabSheet implements View, CloseHandler, ReportEditorListener {
	...
	private Component buildDrafts() {
		...
		Label draftsTitle = new Label("Informes");
	...
	private Component buildDraftThumb() {
		...
		Label draftTitle = new Label("Informe 1<br><span>Subido el 2/11/2015</span>", ContentMode.HTML);
	...
	// public void addReport(final ReportType reportType, final Object
	// prefillData) {
	...
	// }
	//
	// DashboardEventBus.post(new ReportsCountUpdatedEvent(getComponentCount() -
	// 1));
	...
	public void onTabClose(final TabSheet tabsheet, final Component tabContent) {
		...
		Button ok = new Button("Save", new ClickListener() {
			...
			public void buttonClick(final ClickEvent event) {
				...
				Notification.show("The report was saved as a draft",
